...
const AddressCard = ({ 
  ...
  <div className={`relative rounded-[2.5rem] p-8 border-4 transition-all duration-500 overflow-hidden group 
    ${isSelected ? 'border-primary bg-success/10 shadow-2xl scale-[1.02]' : 
      isDefault ? 'border-slate-200 bg-muted' : 'border-slate-50 bg-white hover:border-slate-100 hover:shadow-2xl shadow-slate-200/40'}`}>
    
    {(isDefault || isSelected) && (
      <div className="absolute right-0 top-0 rounded-bl-[1.5rem] bg-primary px-6 py-2 shadow-2xl z-20">
    ...
    <div className="flex items-center gap-4 mb-8">
      <div className={`flex h-12 w-12 items-center justify-center rounded-2xl shadow-inner transition-all duration-500 
        ${(isDefault || isSelected) ? 'bg-primary text-white' : 'bg-slate-100 text-slate-400 group-hover:bg-primary group-hover:text-white'}`}>
        <MapPinned className="h-6 w-6" />
      </div>
      <div className="text-left">
        <h3 className="text-lg font-black text-slate-900 tracking-tight leading-none mb-1">{type}</h3>
        <p className={`text-[10px] font-black uppercase tracking-widest ${(isDefault || isSelected) ? 'text-primary' : 'text-slate-400'}`}>
    ...
    {(onEdit || onDelete || onSetDefault) && (
      <div className="mt-10 flex items-center justify-between relative z-10">
          <div className="flex items-center gap-6">
              {onEdit && <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="text-[10px] font-black uppercase tracking-[0.2em] text-primary hover:text-success transition-colors">Edit</button>}
          ...
          {(!isDefault && onSetDefault) && <button onClick={(e) => { e.stopPropagation(); onSetDefault(); }} className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-primary">Set Default</button>}
      </div>
    )}
  </div>
);
