        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1f2937]/35 backdrop-blur-sm" onClick={onClose}>
            ...
            >
                ...
                <form onSubmit={onSubmit} className="space-y-4 p-6">
                    <div>
                        <label className="text-sm font-medium text-[#1f2937]" htmlFor="product-search">
                            Product
                        </label>
                        {/* <div className="relative mt-1">
                            <input
                                ...
                                autoComplete="off"
                                className="w-full rounded-lg border border-[#e5e7eb] bg-white px-3 py-2 text-sm text-[#1f2937] outline-none focus:ring-2 focus:ring-[#0f5132]"
                            />
                            {products.length > 0 && (
                                <div className="absolute z-10 mt-1 max-h-48 w-full overflow-y-auto rounded-lg border border-[#e5e7eb] bg-white py-1 shadow-lg">
                                    {products.map((product) => (
                                        <button
                                            key={product.id}
                                            type="button"
                                            onClick={() => handleProductSelect(product)}
                                            className="flex w-full flex-col px-3 py-2 text-left text-sm transition-colors hover:bg-[#f3f6f3]"
                                        >
                                            <span className="font-medium text-[#1f2937]">{product.name}</span>
                                            <span className="text-xs text-[#6b7280]">{product.variantIds?.length || 0} variant(s)</span>
                                        </button>
                                    ))}
                                </div>
                            )}
                        </div>
                        {isSearchingProducts && <p className="mt-1 text-xs text-[#6b7280]">Searching products...</p>} */}
                   ...
                   {!inventory && (
                    ...
                    {!inventory && (
                        <div>
                        <label className="text-sm font-medium text-[#1f2937]" htmlFor="warehouse-id">
                            Warehouse
                        </label>
                        <select
                            id="warehouse-id"
