  ...
  if (productsError) {
    return (
      <div className="flex min-h-[500px] flex-col items-center justify-center p-10 text-center bg-white">
        <div className="mb-6 rounded-full bg-destructive p-6 animate-bounce">
          <SlidersHorizontal className="h-10 w-10 text-destructive" />
        </div>
        <h3 className="text-2xl font-black text-foreground">Something went wrong</h3>
        <p className="mt-2 max-w-sm text-muted-foreground">We couldn't load the products. Please check your connection and try again.</p>
        <button 
          onClick={() => window.location.reload()}
          className="mt-8 rounded-full bg-primary px-10 py-3 text-sm font-bold text-white shadow-lg transition-all hover:bg-primary/90"
        >
          Try Again
        </button>
      </div>
      ...
      <div className="bg-white border-b border-border sticky top-0 z-20">
        <div className="mx-auto max-w-screen-2xl px-3 py-4 sm:px-6 lg:px-8">
           <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h1 className="text-xl font-black tracking-tight text-foreground sm:text-3xl">Shop Organic</h1>
                <p className="text-[10px] sm:text-sm font-medium text-muted-foreground">Fresh, healthy essentials for your home.</p>
              </div>
              ...
              <div className="flex items-center gap-2">
                <div className="relative group flex-1 sm:w-80">
                  ...
                  <input
                    ...
                    className="w-full rounded-full border border-border bg-muted py-2 pl-9 pr-3 text-xs sm:text-sm outline-none transition-all focus:border-primary focus:bg-white focus:ring-4 focus:ring-primary/5"
                  />
                </div>
                
                <button 
                  onClick={() => setIsFilterOpen(!isFilterOpen)}
                  className="lg:hidden flex h-9 w-9 items-center justify-center rounded-full bg-primary text-white shadow-lg shadow-success/20"
                >
          ...
          <aside className={`lg:w-72 shrink-0 ${isFilterOpen ? 'fixed inset-0 z-50 bg-white p-6' : 'hidden lg:block'}`}>
            <div className="lg:sticky lg:top-32">
              ...
              <div className="space-y-8">
                {/* Categories */}
                <div>
                  <h3 className="mb-4 text-sm font-black uppercase tracking-wider text-foreground">Categories</h3>
                      ...
                      <button
                        ...
                        className={`flex items-center justify-between rounded-xl px-4 py-2.5 text-sm font-bold transition-all ${
                          categoryFilter === category 
                            ? "bg-primary text-white shadow-md shadow-success/20" 
                ...
                {/* Sort By */}
                <div>
                  <h3 className="mb-4 text-sm font-black uppercase tracking-wider text-foreground">Sort By</h3>
                  <div className="flex flex-col gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={`text-left rounded-xl px-4 py-2 text-sm font-bold transition-all ${
                          sortBy === option.id ? "text-primary" : "text-muted-foreground hover:text-foreground"
                        }`}
                      >
                        {option.label}
                      </button>
          ...
          <main className="flex-1">
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className={`flex h-10 w-10 items-center justify-center rounded-xl transition-all ${
                    viewMode === "grid" ? "bg-white text-primary shadow-sm ring-1 ring-border" : "text-muted-foreground hover:bg-white"
                  }`}
                >
                ...
                <button 
                  ...
                  className={`flex h-10 w-10 items-center justify-center rounded-xl transition-all ${
                    viewMode === "list" ? "bg-white text-primary shadow-sm ring-1 ring-border" : "text-muted-foreground hover:bg-white"
                  }`}
                >
            ...
            {productsLoading ? (
              ...
              <div className="flex min-h-[400px] flex-col items-center justify-center rounded-[3rem] bg-white p-12 text-center shadow-sm">
                ...
                <p className="mt-2 max-w-xs text-muted-foreground">Try adjusting your filters or search terms to find what you're looking for.</p>
                <button 
                  onClick={() => {setSearchTerm(""); setCategoryFilter("All");}}
                  className="mt-8 text-sm font-black text-primary underline decoration-2 underline-offset-4"
                >
                  Clear all filters
                </button>
              </div>
            ...
            {!productsLoading && totalPages > 1 && (
              <div className="mt-12 flex items-center justify-center gap-2">
                <button
                  ...
                  className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-foreground shadow-sm transition-all hover:bg-primary hover:text-white disabled:opacity-30 ring-1 ring-border"
                >
                    ...
                    <button
                      ...
                      className={`h-12 min-w-[48px] rounded-2xl text-sm font-black transition-all ${
                        page === currentPage
                          ...
                          : "bg-white text-muted-foreground hover:bg-gray-50 ring-1 ring-border"
                      }`}
                    >
                      {page}
                    </button>
