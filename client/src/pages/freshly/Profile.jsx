...
import OtpModal from '@/components/freshly/OtpModal';
import OtpPage from '@/components/freshly/OtpPage';
import EnterNewPasswordModal from '@/components/freshly/EnterNewPasswordModal';
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm font-black text-primary uppercase tracking-[0.2em] animate-pulse">Loading Profile...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <aside className="w-full lg:w-80 shrink-0 sticky lg:top-32">
            <div className="overflow-hidden rounded-[3rem] bg-white shadow-2xl shadow-slate-200/60 border border-white">
              <div className="relative bg-primary p-10 text-center text-white overflow-hidden">
                ...
                <div className="relative mx-auto mb-6 h-28 w-28">
                  ...
                  <button className="absolute -bottom-2 -right-2 rounded-2xl bg-yellow-400 p-3 text-primary shadow-2xl hover:scale-110 active:scale-95 transition-all border-4 border-primary">
                    <Edit2 className="h-4 w-4" />
                  </button>
                </div>
                <h2 className="text-2xl font-black tracking-tighter leading-none mb-1">{profile?.first_name} {profile?.last_name}</h2>
                <p className="text-[10px] font-black text-success/60 uppercase tracking-[0.3em] mt-2">Verified Member</p>
              </div>
                ...
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`group flex w-full items-center gap-4 rounded-[1.5rem] px-6 py-4 text-sm font-black transition-all ${
                      activeTab === tab.id 
                        ? "bg-primary text-white shadow-2xl shadow-success/20" 
                        : "text-slate-500 hover:bg-slate-50 hover:text-primary"
                    }`}
                  >
                    <tab.icon className={`h-5 w-5 transition-transform group-hover:scale-110 ${activeTab === tab.id ? "text-yellow-400" : "text-slate-400 group-hover:text-primary"}`} />
                    {tab.label}
                  </button>
                ))}
              ...
              {activeTab === 'personal' && (
                <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
                  <div className="mb-12 flex flex-col sm:flex-row sm:items-end justify-between gap-6">
                    <div>
                      <div className="inline-flex items-center gap-2 rounded-full bg-success px-4 py-1.5 text-[10px] font-black uppercase tracking-widest text-primary mb-4">
                        <User className="h-3.5 w-3.5 fill-primary" />
                        Account Details
                      </div>
                    ...
                    <button className="rounded-full bg-slate-900 px-8 py-4 text-[10px] font-black uppercase tracking-widest text-white hover:bg-primary hover:scale-105 active:scale-95 transition-all shadow-xl shadow-slate-900/10">
                      Edit Profile
                    </button>
                  </div>
                  ...
                  <div className="mt-16 rounded-[2.5rem] bg-slate-50 p-10 border border-slate-100 relative overflow-hidden group">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                        ...
                        <button onClick={()=>{mutate(profile?.phone)}} className="rounded-2xl bg-white border border-slate-200 px-8 py-4 text-xs font-black text-slate-800 hover:border-primary hover:text-primary transition-all shadow-sm">
                            Change Password
                        </button>
                    </div>
                  </div>
                ...
                <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
                  <div className="mb-12 flex flex-col sm:flex-row sm:items-end justify-between gap-6">
                    ...
                    <button 
                      ...
                      className="flex items-center gap-2 rounded-full bg-primary px-8 py-4 text-[10px] font-black uppercase tracking-widest text-white hover:bg-primary/90 hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-success/30"
                    >
              ...
              {['security', 'payment'].includes(activeTab) && (
                <div className="flex flex-col items-center justify-center h-full py-20 text-center animate-in zoom-in duration-500">
                  ...
                  <button 
                    ...
                    className="mt-10 text-xs font-black uppercase tracking-widest text-primary underline decoration-2 underline-offset-8 decoration-yellow-400"
                  >
                    Back to Profile
                  </button>
                </div>
...
const InfoCard = ({ label, value, isVerified }) => (
  <div className="space-y-3 group">
    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1 transition-colors group-hover:text-primary">{label}</label>
    <div className="flex items-center justify-between gap-4 rounded-[1.5rem] bg-slate-50 px-8 py-5 border border-slate-100 transition-all hover:bg-white hover:border-primary/30 hover:shadow-xl hover:shadow-slate-200/40">
      <span className="text-sm font-black text-slate-900">{value}</span>
      {isVerified && (
        <div className="flex items-center gap-1.5 rounded-full bg-success px-3 py-1">
            <CheckCircle2 className="h-3 w-3 text-success" />
            <span className="text-[8px] font-black uppercase tracking-wider text-success">Verified</span>
        </div>
      )}
    </div>
  </div>
...
const AddressCard = ({ type, isDefault, name, address, city, state, pincode, phone, onEdit, onDelete, onSetDefault }) => (
  <div className={`relative rounded-[2.5rem] p-8 border-4 transition-all duration-500 overflow-hidden group ${isDefault ? 'border-primary bg-success/10' : 'border-slate-50 bg-white hover:border-slate-100 hover:shadow-2xl shadow-slate-200/40'}`}>
    {isDefault && (
      <div className="absolute right-0 top-0 rounded-bl-[1.5rem] bg-primary px-6 py-2 shadow-2xl">
    ...
    <div className="flex items-center gap-4 mb-8">
      <div className={`flex h-12 w-12 items-center justify-center rounded-2xl shadow-inner transition-all duration-500 ${isDefault ? 'bg-primary text-white' : 'bg-slate-100 text-slate-400 group-hover:bg-primary group-hover:text-white'}`}>
        <MapPinned className="h-6 w-6" />
      </div>
      <div>
        <h3 className="text-lg font-black text-slate-900 tracking-tight leading-none mb-1">{type}</h3>
        <p className={`text-[10px] font-black uppercase tracking-widest ${isDefault ? 'text-primary' : 'text-slate-400'}`}>Primary Delivery</p>
      </div>
    </div>
    ...
    <div className="space-y-2 relative z-10">
      ...
      <div className="flex items-center gap-2 mt-4 text-xs font-black text-primary">
          <span className="px-2 py-0.5 rounded bg-primary/10">TEL</span>
          {phone}
      </div>
    </div>
    ...
    <div className="mt-10 flex items-center justify-between relative z-10">
        <div className="flex items-center gap-6">
            <button onClick={onEdit} className="text-[10px] font-black uppercase tracking-[0.2em] text-primary hover:text-success transition-colors">Edit</button>
        ...
        {!isDefault && <button onClick={onSetDefault} className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-primary">Set Default</button>}
    </div>
  </div>
);
