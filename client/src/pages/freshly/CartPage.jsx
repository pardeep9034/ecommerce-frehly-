...
import AddressCard from "@/components/freshly/AddressCard";
...
placeOrder.mutate({
  ...
  if (cartItems.length === 0 && step === 1) {
    return (
      <div className="flex min-h-[70vh] flex-col items-center justify-center p-6 text-center">
        ...
        <Link 
          to="/shop" 
          className="mt-10 inline-flex items-center gap-2 rounded-full bg-primary px-8 py-4 text-sm font-black uppercase tracking-widest text-white shadow-2xl shadow-success/30 transition-all hover:scale-105 hover:bg-primary/90 active:scale-95"
        >
          Start Shopping
  ...
  return (
    <div className="min-h-screen bg-muted pb-24 pt-8">
              ...
              <div key={s.id} className="relative z-10 flex flex-col items-center group">
                <div 
                  className={`flex h-12 w-12 items-center justify-center rounded-full border-4 transition-all duration-500 ${
                    step >= s.id 
                      ? "bg-primary border-primary text-white shadow-lg shadow-success/20" 
            ...
            {step === 1 && (
              <div className="space-y-6">
                <div className="flex items-center justify-between pb-4 border-b border-slate-200">
                  ...
                  <button onClick={() => {}} className="text-xs font-black uppercase tracking-widest text-destructive hover:text-destructive transition-colors">Clear Bag</button>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  {cartItems.map((item) => (
                    <div 
                      ...
                      className="group relative flex gap-4 rounded-[2rem] border border-slate-100 bg-white p-4 transition-all hover:border-success/30 hover:shadow-xl hover:shadow-success/5 sm:gap-6 sm:p-6"
                    >
                      {/* Product Image */}
                      ...
                      <div className="flex flex-1 flex-col justify-between py-1">
                        <div>
                          <div className="flex items-start justify-between">
                            <h3 className="text-sm font-black text-slate-900 sm:text-lg tracking-tight group-hover:text-primary transition-colors text-left">{item.product_name}</h3>
                            <button 
                              onClick={() => handleRemoveItem(item.product_id, item.variant_id)}
                              className="rounded-full p-2 text-slate-400 transition-all hover:bg-destructive hover:text-destructive"
                            >
                              <Trash2 className="h-5 w-5" />
                            </button>
                          </div>
                          <p className="mt-1 text-[10px] font-black uppercase tracking-widest text-primary/60 text-left">{item.variant_name}</p>
                        </div>
            ...
            {step === 2 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-left">
                  ...
                  <button onClick={()=>{setAddressModalOpen(true)}} className="flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-[10px] font-black uppercase tracking-widest text-white hover:bg-primary/90 transition-all shadow-xl shadow-success/10">
              ...
              <div className="p-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between text-sm font-bold text-slate-500">
                    ...
                    <span className={deliveryFee === 0 ? "text-success" : "text-slate-900"}>
                ...
                <div className="mt-8 space-y-4">
                  {step === 1 && (
                    <button 
                      onClick={() => setStep(2)}
                      className="w-full flex h-14 items-center justify-center gap-3 rounded-full bg-primary px-8 text-sm font-black uppercase tracking-widest text-white shadow-xl shadow-success/20 transition-all hover:bg-primary/90 hover:scale-[1.02] active:scale-95 group"
                    >
                      Checkout Now
