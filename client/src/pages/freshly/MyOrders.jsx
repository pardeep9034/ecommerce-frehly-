  ...
  if (isError) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-muted p-4 text-center">
        ...
        <Link to="/shop" className="mt-6 font-medium text-primary hover:underline">
          Return to Shop
        </Link>
      </div>
    );
      ...
      <div className="mx-auto max-w-4xl px-4">
        ...
        {orderHistory.length > 0 ? (
                    ...
                    {order.items.map((item) => (
                      <div key={item.id} className="flex items-center gap-4 py-4 first:pt-0 last:pb-0">
                        ...
                        <div className="text-right">
                          <p className="text-sm font-bold text-primary">₹{item.line_total}</p>
                ...
                <div className="border-t border-gray-50 bg-gray-50/30 px-6 py-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-lg font-extrabold text-primary">₹{order.total_amount}</p>
                    </div>
                    <Link 
                      to={`/orders/${order.id}`}
                      className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-primary/90"
                    >
                      Order Details
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-200 bg-white p-12 text-center">
            ...
            <Link 
              to="/shop" 
              className="mt-8 rounded-xl bg-primary px-8 py-3 text-sm font-bold text-white shadow-lg shadow-primary/20 transition-all hover:scale-105 hover:bg-primary/90"
            >
              Start Shopping
            </Link>
          </div>
