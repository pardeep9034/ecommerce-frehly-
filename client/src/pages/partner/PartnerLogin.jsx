...
// Mirrors pages/dashboard/AdminLogin.jsx: partners get their own login route
// (like staff do) instead of branching role logic into the shopper-facing
// LoginPage, same reason AdminLogin exists separately from it.
const PartnerLogin = () => {
  ...
  const { mutate: signIn, isPending } = useMutation({
    ...
    onSuccess: (response) => {
      ...
      if (getTokenRole(accessToken) !== "DELIVERY_PARTNER") {
        setError("This account isn't a delivery partner account.");
        return;
      }
    ...
    <main className="min-h-screen bg-muted p-4 sm:p-8">
      <div className="mx-auto grid min-h-[calc(100vh-2rem)] max-w-5xl overflow-hidden rounded-3xl bg-card shadow-[0_24px_60px_-24px_hsl(var(--primary)/0.32)] lg:grid-cols-[1.05fr_0.95fr] sm:min-h-[calc(100vh-4rem)]">
        <section className="hidden flex-col justify-between bg-primary p-10 text-white lg:flex">
          ...
          <div>
            <p className="mb-3 text-sm font-semibold uppercase tracking-[0.2em] text-accent/70">Partner Portal</p>
            <h1 className="max-w-sm text-4xl font-bold leading-tight">Deliver with Freshly.</h1>
            <p className="mt-5 max-w-sm text-base leading-7 text-white/75">
              See today's assignments, update delivery status, and track your history.
            </p>
          </div>

          <p className="text-sm text-white/55">Freshly Delivery Partners</p>
        </section>
        ...
        <section className="flex items-center justify-center p-6 sm:p-10">
          <div className="w-full max-w-sm">
            <div className="mb-9 lg:hidden">
              ...
              <p className="text-sm font-semibold uppercase tracking-[0.16em] text-warning">Partner Portal</p>
            </div>

            <div className="mb-8">
              <h2 className="text-3xl font-bold tracking-tight text-foreground">Welcome back</h2>
              <p className="mt-2 text-sm leading-6 text-muted-foreground">Sign in to see your deliveries.</p>
            </div>
            ...
            <form className="space-y-5" onSubmit={handleSubmit} noValidate>
              ...
              >
                {isPending ? "Signing in..." : "Sign in to partner portal"}
              </button>
            </form>
          </div>
        </section>
