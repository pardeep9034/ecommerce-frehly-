...
class addToCartService {
  async add(user, data, warehouse_id) {
    ...
    const existingCartItem = await CartItemRepository.checkCartItem({
      cart_id: cart.id,
  ...
  async validateProductAndInventory(data, warehouseId) {
    const [productResponse, inventoryResponse] = await Promise.all([
      ...
      axios.get(`${env.API_GATEWAY_URL}/inventory/variant/${data.variant_id}`, {
        headers: {
    ...
    if (!productResponse.data.data || !productResponse.data.success) {
      return {
        success: false,
        message:
          productResponse?.data?.message || "product service unavailable",
      };
    }
    ...
    if (!inventoryResponse.data.data || !inventoryResponse.data.success) {
      return {
        success: false,
        message:
          inventoryResponse?.data?.message || "inventory service unavailable",
      };
    }
  ...
  async handleExistingCartItem(existingCartItem, inventory, data) {
    ...
    // update quantity
    const updateQuantity = await CartItemRepository.updateQuantity(
      existingCartItem.id,
      finalQuantity,
    );
 ...
 async cartByUserId(userId) {
  ...
  // Convert Sequelize instance → plain JS object
  ...
  const itemInfoResponse = await fetch(
    `${env.API_GATEWAY_URL}/product-variant/variants/by-ids`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!itemInfoResponse.ok) {
    throw new AppError(
      "Error occurred during fetching item info",
      502
    );
  }
  ...
  async increaseQuantity(userId, cartItemId,warehouseId) {
    const cart = await cartRepository.getCartByUserId(userId);
    ...
    const variantInventoryResponse = await fetch(
      `${env.API_GATEWAY_URL}/inventory/variant/${cartItem.variant_id}`,{
         method:"GET",
        headers:{
            "x-warehouse-id":warehouseId
        }
    ...
    const remainingStock =
      number(vaiantInventory.current_stock) -
      number(vaiantInventory.reserved_stock);
    if (remainingStock >= 1) {
      cartItem.quantity += 1;
      await cartItem.save();
    }
  ...
  async decreaseQuantity(userId, cartItemId,warehouseId) {
    ...
    const variantInventoryResponse = await fetch(
      `${env.API_GATEWAY_URL}/inventory/variant/${cartItemData.variant_id}`,{
        method:"GET",
        headers:{
            "x-warehouse-id":warehouseId
        }
  ...
  async mergeCarts(userId, guestCart, warehouseId) {
    ...
    if (!userCart) {
      await this.createCart({ user_id: userId });
    }
    ...
    const inventoryResponse = await fetch(
      `${env.API_GATEWAY_URL}/inventory/validate`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    await sequelize.transaction(async (transaction) => {
      await Promise.all(validUpdates.map((item) => item.save({ transaction })));
    ...
    if (removedItems.length > 0) {
      //publish event
      await publisher.publish(cartEvents.CART_ITEM_REMOVED, {
  ...
  async clearCart(userId) {
    const cartModel = await this.cartByUserId(userId);
    const cart = await cartModel.toJSON();
    if (cart) {
      await CartItemRepository.destroy({
        where: {
          cart_id: cart.id,
